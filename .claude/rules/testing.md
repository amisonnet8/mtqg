...
- `archive`：**行を移す2つ目のコマンドなので、行が失われないことを確かめる。** 移した行がアーカイブに元のバイト列のまま入り、残る行も元のバイト列のまま、移した行と残した行を合わせると元の行の全部になる、`cat archive >> journal.jsonl`で元の状態に戻る、読めなかった行は移らない、`-n`は1バイトも書かず`archive/`も作らない、移す行が0なら`archive/`を作らない、衝突マーカーがあれば何も書かない、`移す＋残す`が合わなければ何も書かない。期間は両端の日を含むこと（ローカルの日付）、質問・バグは回答・返信の日付まで含めて判定すること、削除した記録は種類を問わず移ること、未完了のtodo・質問・bugとglossaryは移らないこと、親のない返信はmemoと同じに扱うこと。アーカイブと追記を同時に走らせても行が消えない。壊して確かめる変異：終了日を含めない、UTCで判定する、未完了のtodoも移す、glossaryも移す、回答を親と一緒に移さない、回答の日付を見ない、削除した記録を移さない、`-n`が書く、読めなかった行も移す、行数の検査を外す
