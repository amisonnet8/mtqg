...
- `context`：予算に収まる、削る順序が仕様どおり、空の区画を出さない、削ったものを区画の中で言う。**削る順序は、予算を少しずつ下げながら、後の削りが先に起きないことを不変条件で確かめる**（「この予算ならこうなる」という数字を固定すると、文言を1つ直すたびに壊れる）。予算を無視する、todoを新しい順に削る、定義を残して質問を削る、完了したtodoが混ざる、の変異で確かめた
