...
**段階1 Step 6（`edit`・`delete`・`undo`・`search`・`review`・`format`、`tty`）：仕様（`cli.md`・`cli_ja.md`、ルール類、`docs/design/`）を書いた。実装はこれから（2026-09-21）。** Step 5はCIの3OSの確認待ちのまま（人間がpushして確認）。

**Step 6で決めたこと（この会話で確認済み。理由は`docs/design/history.md`）：** ①種類をまたぐ`re`の行は**案C**（読み方は今のまま、`show`の文言を直し、`review`に「親のない回答・返信」の区画を足す）。②`$EDITOR`は**書く本文が無いときすべて**で開く（`edit <id>`は今の本文を入れて、`q add <質問id>`、`b add <バグid>`、`g add <用語>`）。③`undo`が断るのは、**`create`を消すと他のイベントが記録なしで残るときだけ**。④`tty`は**端末のデバイス番号のハッシュ、`MTQG_TTY`を今入れる**（Windowsと端末のないときは値なし）。計画の細部で、実装前に決めたこと：`undo`の「最後の行」は`ts`が最後、同じ秒ならファイルで最後（`id`はランダムなので同じ秒の並びに使えない）。並行した状態変更の判定は、その時点の状態と`from`の比較（同じ`from`が2つ、では正しい順番の変更を誤検出する）。`format`のファイルの読み込みは`Env.ReadFile`から行う。

**（前の状態）段階1 Step 5（`context`と全コマンドの`--json`）：実装と手元の検証が済んだ（2026-09-21）。CIの3OSの確認待ち（人間がpushして確認）。次はStep 6。** これで**AIに渡せる**：`mtqg context`をセッションの始めに読ませ、`--json`で拡張やスクリプトがつながる。サンプルPJ（段階2）でAIにも使わせられる。
