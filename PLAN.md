...
1. **qsokuのリポジトリの初期構成を作る。** そのリポジトリを担当するClaude Codeが、リポジトリの目的をすぐ理解できるようにする（CLAUDE.md相当の指示ファイル、`mtqg context`を作業開始時に読む・質問や決定が生じたら記録する、という指示を含む）。**決定済み**：モジュールパスは`github.com/amisonnet8/qsoku`。公開範囲はmtqg本体と同じ「公開の2段階」方針（最初からpublic、タグは打たない。未完成の間は注意書きだけのREADME）。mtqgの入れ方は**人間が用意する**（devcontainerビルド後にこのリポジトリをclone）。報告の形式は設計§12.3の「CLIで直すもの」「11章で解くもの」の2分類。**まだ着手していない。初期に何を作るかは人間から指示が来るまで待つ**（2026-09-23、人間の指示）
2. **開発後の報告を受け取り、ここに記録する。** 終わったら、出た問題を`docs/design/08-development.md`「12.3 サンプルPJ」の2分類（CLIで直すもの→段階3で吸収／11章で解くもの→段階4の要否の材料）で受け取り、段階3の着手判断に使う

## 現在地

**段階1 Step 9（シェル補完）：完了（2026-09-21、CIの3OSがgreen。人間が確認）。これで段階1のステップはすべて終わった。v0.1のタグは打たない（人間の判断、2026-09-21。下の段階1完了の判定のところに記録）。段階2（サンプルPJ）に進むと決定、題材はqsoku（2026-09-23。詳細は「段階2：サンプルPJ」の節を参照）。初期構成に必要な決定（モジュールパス・公開範囲・mtqgの入れ方・報告の形式）は済んだ。次は、人間から初期に何を作るかの指示が来るのを待つ（着手はまだ）。**
