...
**題材を決定（2026-09-23、人間の判断）：qsoku——リポジトリ固有のコマンドの近道を`qsokufile`に書き、どこにいても`qsoku <名前>`で呼べるCLI。** 置き場所は`/home/vscode/qsoku`、設計メモは`/home/vscode/qsoku/qsoku-note.md`。

**選定の経緯：** 最初は日本の営業日計算（祝日対応）を選んだが（設計§12.3の基準に沿って提案・決定）、仕様がいったん決まるとフラグに従って日付をフィルタするだけになり、判断のほとんどが実装前の話し合いに前倒しになって「実装しながら迷いどころが出る」度合いが低いと人間が判断（2026-09-22〜23）。アルゴリズム視覚化ツール、Goコードの可視化ツールも検討したが、最終的に**qsoku**に決定。理由（設計メモに記載）：外部データへの依存が無い（amicalの弱点の裏返し）、`//`の置き換え規則・`cd`の持ち帰り方・`sh`固定の理由など、実装しながら決めた・直したことがすでにメモの時点で多く記録されている、シェル補完（mtqgのStep 9と比較材料になる。qsokuはPowerShell対象外、bash・zsh・fishのみ）。**「本当に便利な道具になりそうなので、記録は使い捨てにせず残す方向」**（設計メモ）。

**リポジトリは分ける。** このリポジトリ（mtqg本体）ではコードを書かない。qsokuのリポジトリでの開発は、このリポジトリの外（別セッション、`/home/vscode/qsoku`）で行われる。ここでの役割は2つ。
...
**CIも3OS全ジョブgreen（PR #2、2026-09-24、人間が確認）。mainへマージ済み。**
...
**qsokuの報告「11章で解くもの」の残り2件に着手・完了した（2026-09-25）。** 人間と方針を確認（`mtqg q` `f5d40bd4fe`）：①memoとbugの使い分けは文書・ルールの強化だけにする。②手作業の変異確認の機械化は、AST変異を自動生成するツールを`.claude/skills/`に作る。

1. **memo/bugの使い分け：** `.claude/rules/mtqg-usage.md`の`b add`行に判定基準（「本来動くべきものが動いていなかった」「仕様と実装がずれていた」はbug、単なる気づきはmemo。「直したから」は理由にならない）を明記し、MCPの`memo_add`・`bug_report`ツールの説明文にも同じ基準を（互いを参照する形で）追記した。フックでの能動的な促しは作らない（誤検知のリスクと、人間が確認済みの対策範囲を越えるため）。CLIの`--help`要約・`docs/reference/`は変えていない（使い方の指南であって仕様の契約ではないため、CLAUDE.md「ドキュメントの書き方」の線引きに従った）。
2. **手作業の変異確認の機械化：** `.claude/skills/mutation-check/gen-mutants/`（mtqg本体とは別のGoモジュール、標準ライブラリのみ）を新設した。`go/parser`でファイルを解析し、比較・論理・算術演算子の反転、真偽値リテラルの反転、先頭`!`の削除、整数リテラルの境界値（±1）という、構文だけで判定できる変異を自動で列挙し、1つずつ入れてテストコマンドを回し、`mutate.sh`と同じ語彙（killed/SURVIVED/not compiled）で報告する。仕様の意味を読まないと判定できない変異（`mutate.sh`が引き続き担当）は代替しない。`internal/cli/candidates.go`で動作確認：90変異中78 killed・6 not compiled・6 SURVIVED（うち1件は境界で結果が同じになる等価変異と確認）。詳細・使い方は`.claude/skills/mutation-check/SKILL.md`、`.claude/rules/testing.md`。

11章の外部ツール連携（設計§11）のうち残るのはVSCode拡張（設計§11.4、別リポジトリの方向）だけ。v1確定（設計§12.4）に進むかどうかは人間の判断。

## 現在地

**`v0.3.0`のタグをローカルで作成した（2026-09-26、コミット`134142e`、人間の判断・指示）。** `v0.2.0`以降の変更（`$EDITOR`未設定時のnanoフォールバック、CIフレーク3件の修正、`--at`・`log --before`・`log --json --events`の3新機能。すべて下位互換）をまとめた区切り。タグの内容は`git tag -n999 v0.3.0`で見られる（release note風のannotated tag）。**pushはまだしていない**（人間が行う）。pushされて初めて`go install .../mtqg@v0.3.0`が使えるようになる。
...
**リリース前後に「様子見」で開いていたCIフレーク2件と、その調査中に見つけた同型の潜在リスク1件を、すべて調査・修正した（2026-09-26）。** インフラの偶発事象ではなく、テスト側の設計に原因があった：①`TestRewriteWhileGoroutinesAppend`（`internal/journal/rewrite_test.go`）が、追記側の決めた回数（4×30）に対し書き直し側の「3回以上」を事後に確かめるだけの古い形のままで、CIで3回flakeした（`only 1 rewrites ran while appending`。windows-latest・race macos-latest）。兄弟の`TestArchiveWhileGoroutinesAppend`が既に採っている「追記は書き直しが必要な回数を終えるまで続ける」形に直して解消。②`e2e/examples_test.go`の`copyOf`が、フィクスチャのgitリポジトリを`filepath.WalkDir`でコピーする際、gitが`.git/objects/`の中に作って消す一時ファイル（`maintenance.lock`）に競合してmacos-latestで1回落ちた。一覧に載ってから読むまでに消えたファイルは飛ばすようにし、決定的な回帰テスト（`TestCopyEntrySkipsVanishedFiles`・`TestCopyEntrySkipsFilesThatVanishAfterStat`）を追加、mutation-checkで両方killedを確認。③調査の過程で見つけた同型の潜在リスク`TestRewriteWhileProcessesAppend`（プロセス版）も、思ったより軽く直せると判り、当日中に修正した：`TestHelperProcess`の`append`モードに「`MTQG_COUNT`が0以下なら`kill`されるまで無限に書き続ける」を足し、書けた件数はプロセスに報告させず`journal.jsonl`から直接数える形にした。詳細は`.claude/rules/testing.md`（「並行テストには『証明にならない』ガードを入れる」「GitHub Actions CIの落とし穴」）。
...
**段階4b（`mtqg mcp`）を実装した（2026-09-25。上の「段階4b」の節）。** 人間の指示とプランモードでの合意を経て、公式SDKを依存ポリシーの例外として採用し、18ツールを`internal/cli/mcp.go`・`mcp_tools.go`に実装した。手元の検証（`qsoku check`・`qsoku test`・`qsoku race`・`qsoku shellcheck`・`qsoku trivy`、mutation-check）はすべて済んだ。**CIも3OS全ジョブgreen（PR #5、mainへマージ済み、`4df2674`）。**
...
- **タグは、バージョンを指定して`go install`できるようにするためだけの操作であり、「完成として公開する」（下の表の段階を進める）とは別の判断。** 元々はv0.1まで一切打たない方針だったが、**`v0.2.0`からタグを打つことにした（人間の判断、2026-09-25。`.claude/rules/distribution.md`）**。タグを打つとGoのモジュールプロキシ（`proxy.golang.org`）にバージョンが記録され、後から消せないため、それでも「公開中・未完成」のまま（READMEは注意書きのまま、GoReleaser・GitHub Releasesは作らない）でよいかは、タグを打つたびに人間が判断する
