...
**段階1 Step 9（シェル補完）：完了（2026-09-21、CIの3OSがgreen。人間が確認）。これで段階1のステップはすべて終わった。v0.1のタグは打たない（人間の判断、2026-09-21。下の段階1完了の判定のところに記録）。段階2（サンプルPJ）に進むと決定、題材はqsoku（2026-09-23。詳細は「段階2：サンプルPJ」の節を参照）。qsokuリポジトリの初期構成を作成済み（2026-09-23、`/home/vscode/qsoku`、コミット`6716c3a`。まだpushしていない）。次は、qsoku側の開発が進み、その報告（CLIで直すもの／11章で解くもの）を受け取るのを待つ。**
