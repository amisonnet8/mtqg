...
**題材を決定（2026-09-22、人間の判断）：日本の営業日計算（祝日対応）のライブラリ＋CLI。** 選定は設計§12.3の基準（セッションをまたぐ、質問・判断・用語の合意が自然に生まれる、AIエージェントと一緒に作る、ブランチ・worktree・別端末で並行して記録する、規模は数日〜2週間）に沿って、このセッションでmtqgに提案し決めた。祝日の扱い（年末年始・振替休日・国民の休日・春分秋分）に迷いどころが多く、質問（qa）と用語（glossary、日本語）が自然に生まれる見込み。

**リポジトリは分ける。** このリポジトリ（mtqg本体）ではコードを書かない。ここでの役割は2つ。

1. **サンプルPJのリポジトリの初期構成を作る。** そのリポジトリを担当するClaude Codeが、リポジトリの目的をすぐ理解できるようにする（CLAUDE.md相当の指示ファイル、`mtqg context`を作業開始時に読む・質問や決定が生じたら記録する、という指示を含む）。**着手前に決めること**（未着手）：置き場所（隣のディレクトリに新規作成、またはこのリポジトリの一時領域に作って人間が移す）、リポジトリ名・公開範囲、Goのモジュールパス、mtqgの入れ方（未タグなので`go install ...@main`かビルド済みバイナリを置くか）、報告の形式（設計§12.3の「CLIで直すもの」「11章で解くもの」の分類に合わせる）
2. **開発後の報告を受け取り、ここに記録する。** サンプルPJのリポジトリでの開発は、このリポジトリの外（別セッション）で行われる。終わったら、出た問題を`docs/design/08-development.md`「12.3 サンプルPJ」の2分類（CLIで直すもの→段階3で吸収／11章で解くもの→段階4の要否の材料）で受け取り、段階3の着手判断に使う

**まだ着手していない。** サンプルPJのリポジトリはまだ存在しない。

## 現在地

**段階1 Step 9（シェル補完）：完了（2026-09-21、CIの3OSがgreen。人間が確認）。これで段階1のステップはすべて終わった。v0.1のタグは打たない（人間の判断、2026-09-21。下の段階1完了の判定のところに記録）。段階2（サンプルPJ）に進むと決定（2026-09-22。題材は「段階2：サンプルPJ」の節を参照）。次はサンプルPJのリポジトリの初期構成づくり（未着手）。**
